...
abstract class Khcn_Db_Export
{
  ...
  public function getAdapter()
  {
    if( null === $this->_adapter ) {
      throw new Khcn_Exception('No Db Adapter');
    }
  ...
  public function setListener(Khcn_Observer_Interface $listener)
  {
  ...
  public function write($file)
  {
    if( is_string($file) ) {
      if( !($handle = fopen($file, 'w')) ) {
        throw new Khcn_Exception(sprintf('Unable to write to file: "%s"'), $file);
      }
      $this->_handle = $handle;
    } else if( is_resource($file) ) {
      $this->_handle = $handle = $file;
    } else {
      throw new Khcn_Exception(sprintf('Unable to write to file, given: "%s"'), gettype($file));
    }
