<?php
// Khcn/Filter/File/Resize.php
/**
 * Zend Framework addition by Khcn
 * 
 * @category   Khcn
 * @package    Khcn_Filter
 ...
 * @package    Khcn_Filter
 */
class Khcn_Filter_File_Resize implements Zend_Filter_Interface
{
    ...
    protected $_adapter = 'Khcn_Filter_File_Resize_Adapter_Gd';
     ...
     * @return Khcn_Filter_File_Resize An instance of this filter
     */
    public function __construct($options = array())
    {
        ...
        if (isset($options['adapter'])) {
            if ($options['adapter'] instanceof Khcn_Filter_File_Resize_Adapter_Abstract) {
                $this->_adapter = $options['adapter'];
            } else {
                $name = $options['adapter'];
                if (substr($name, 0, 33) != 'Khcn_Filter_File_Resize_Adapter_') {
                    $name = 'Khcn_Filter_File_Resize_Adapter_' . ucfirst(strtolower($name));
                }
                $this->_adapter = $name;
            }
        }
    ...
    {
        if ($this->_adapter instanceof Khcn_Filter_File_Resize_Adapter_Abstract) {
            return;
        } else {
